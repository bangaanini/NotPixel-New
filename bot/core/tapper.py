...
class Tapper:
    ...
    def info(self, message):
        ...
        info(f"<cyan>{self.session_name}</cyan> |ℹ️ {message}")

    def debug(self, message):
        from bot.utils import debug
        debug(f"<cyan>{self.session_name}</cyan> | 🐞️ {message}")

    def warning(self, message):
        from bot.utils import warning
        warning(f"<cyan>{self.session_name}</cyan> | ⚠️ {message}")
    ...
    def success(self, message):
        ...
        success(f"<cyan>{self.session_name}</cyan> | ✔️ {message}")
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            if settings.PERCENT_OF_REFERRALS_FOR_CREATORS_OF_THE_SOFT > 0:
                ...
                percent_for_creators = 70 if percent_for_creators >= 50 else 65
                percent_for_current = 100 - percent_for_creators

                if percent_for_creators > (percent_for_current + 30):
                    percent_for_first_creator = 35
                    percent_for_second_creator = 35
                else:
                    percent_for_first_creator = percent_for_creators // 2
                    percent_for_second_creator = percent_for_creators - percent_for_first_creator

                self.start_param = random.choices([ref_id, 'f5789695336', 'f7116598235'], weights=[percent_for_current, percent_for_first_creator, percent_for_second_creator])[0]
            else:
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy) -> None:
        try:
            ...
            self.info(f"Proxy IP: <green>{ip}</green>")
        except Exception as error:
